...
@section('breadcrumb')
    <nav class="flex items-center space-x-1 text-sm">
        ...
        <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
        <a href="{{ route('assignment.show', ['classroom' => $classroom, 'assignment' => $assignment]) }}" class="text-gray-500 hover:text-indigo-600 transition-colors">{{ $assignment->title }}</a>
        <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div class="space-y-2 pt-2">
                        <button wire:click="grade"
                            ...
                            <span wire:loading wire:target="grade"><i class="fas fa-spinner fa-spin mr-1"></i> {{ __('Saving...') }}</span>
                        </button>
                    ...
                    <div class="mt-2 p-3 bg-green-50 rounded-lg text-center">
                        ...
                        <p class="text-sm text-green-700 font-medium">{{ __('Graded') }} {{ $submission->graded_at?->translatedFormat('j M, H:i') }}</p>
                    </div>
                    @endif
                </div>
            </div>
