...
<a href="{{ route('assignment.show', ['classroom' => $classroom, 'assignment' => $assignment]) }}" wire:navigate
    class="group block rounded-lg border border-[#dedee5] bg-white shadow-[rgba(0,0,0,0.03)_0px_4px_24px] p-5 transition hover:border-[rgba(37,99,235,0.3)] hover:shadow-[rgba(0,0,0,0.06)_0px_4px_24px]">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <div class="min-w-0">
                    <h4 class="truncate text-lg font-semibold text-[#101114] group-hover:text-(--ll-blue)">{{ $assignment->title }}</h4>
        ...
        <div class="flex shrink-0 flex-col items-start gap-2 sm:items-end">
            ...
            @else
                <span class="inline-flex items-center gap-2 rounded-md px-3 py-1 text-xs font-semibold {{ $isCompleted ? 'bg-[rgba(20,158,97,0.16)] text-[#026b3f]' : 'bg-[rgba(104,107,130,0.12)] text-[#484b5e]' }}">
