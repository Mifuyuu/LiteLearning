...
@php
    ...
    $dropdownFilters = [
        ...
        [
            ...
            'current' => $filterType,
            'allLabel' => 'ทุกประเภท',
            'options' => $types->map(fn ($t) => ['value' => $t, 'label' => ucfirst($t)]),
            'menuClass' => 'w-44',
        ],
    ];
@endphp
...
<div class="max-w-4xl mx-auto">
    <section class="rounded-2xl border-3 border-[#dedee5] bg-white shadow-[rgba(0,0,0,0.03)_0px_4px_24px] overflow-hidden min-h-[calc(100vh-3rem)] flex flex-col">

        {{-- Header --}}
        <div class="border-b border-[#dedee5] p-6">
        ...
        {{-- Stats --}}
        <div class="grid gap-3 border-b border-[#dedee5] p-5 sm:grid-cols-2 sm:p-6 xl:grid-cols-4">
            <div class="rounded-lg border border-[#dedee5] bg-[#f9f9fb] p-4">
                <p class="text-xs font-bold uppercase text-[#9497a9]">งานที่มอบหมาย</p>
                <p class="mt-2 text-2xl font-bold text-[#101114]">{{ $stats['assignment_count'] }}</p>
            </div>
            <div class="rounded-lg border border-[#dedee5] bg-[#f9f9fb] p-4">
                <p class="text-xs font-bold uppercase text-[#9497a9]">อัตราการส่งงาน</p>
                <p class="mt-2 text-2xl font-bold text-[#101114]">{{ $stats['submission_rate'] }}%</p>
            </div>
            <div class="rounded-lg border border-[#dedee5] bg-[#f9f9fb] p-4">
                <p class="text-xs font-bold uppercase text-[#9497a9]">คะแนนเฉลี่ย</p>
                <p class="mt-2 text-2xl font-bold text-[#101114]">{{ $stats['avg_score'] !== null ? $stats['avg_score'].'%' : '—' }}</p>
            </div>
            <div class="rounded-lg border border-[#dedee5] bg-[#f9f9fb] p-4">
                <p class="text-xs font-bold uppercase text-[#9497a9]">รอตรวจ</p>
                <p class="mt-2 text-2xl font-bold text-[#101114]">{{ $stats['pending_grading'] }}</p>
            </div>
        </div>

        {{-- Filters --}}
        <div class="flex flex-wrap items-center gap-3 border-b border-[#dedee5] p-5 sm:p-6">
            ...
            @foreach($dropdownFilters as $filter)
                ...
                <div class="dropdown">
                    <div tabindex="0" role="button" class="{{ $inputClass }} inline-flex cursor-pointer items-center gap-2">
                        <span>{{ $filter['current'] !== '' ? ($filter['options']->firstWhere('value', $filter['current'])['label'] ?? $filter['current']) : $filter['allLabel'] }}</span>
                        <x-icon name="chevron-down" class="h-4 w-4 text-[#9497a9]" />
                    </div>
                    <ul tabindex="0" class="dropdown-content z-10 mt-1 {{ $filter['menuClass'] }} rounded-lg border border-[#dedee5] bg-white p-1 shadow-lg">
                        <li>
                            <button type="button" wire:click="$set('{{ $filter['field'] }}', '')" onclick="document.activeElement.blur()"
                                class="w-full rounded-md px-3 py-2 text-left text-sm hover:bg-[#f7f7fa] {{ $filter['current'] === '' ? 'font-bold text-(--ll-blue)' : 'text-[#101114]' }}">
                                {{ $filter['allLabel'] }}
                            </button>
                        </li>
                        @foreach($filter['options'] as $option)
                            <li>
                                <button type="button" wire:click="$set('{{ $filter['field'] }}', @js($option['value']))" onclick="document.activeElement.blur()"
                                    class="w-full truncate rounded-md px-3 py-2 text-left text-sm hover:bg-[#f7f7fa] {{ $filter['current'] === $option['value'] ? 'font-bold text-(--ll-blue)' : 'text-[#101114]' }}">
                                    {{ $option['label'] }}
                                </button>
                            </li>
                        @endforeach
