<div class="animate__animated animate__fadeIn">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div class="flex items-center space-x-3 mb-2">
            ...
            <h2 class="text-2xl font-bold text-gray-900">{{ __('Complete your profile setup') }}</h2>
        </div>
        <p class="text-gray-500 mb-6">{{ __('Please select your role before using the platform.') }}</p>

        <form wire:submit="completeSetup" class="space-y-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('Select your role') }}</label>
                <div class="grid grid-cols-2 gap-4">
                    <label class="relative cursor-pointer">
                        ...
                        <div
                            ...
                            <span class="text-sm font-semibold">{{ __('Student') }}</span>
                        </div>
                    </label>
                    <label class="relative cursor-pointer">
                        ...
                        <div
                            ...
                            <span class="text-sm font-semibold">{{ __('Teacher') }}</span>
                        </div>
                    </label>
                </div>
            ...
            <button type="submit"
                ...
                <span wire:loading.remove wire:target="completeSetup">{{ __('Get Started') }}</span>
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-6 text-center">
            @csrf
            <button type="submit"
                class="text-sm text-gray-500 hover:text-gray-700 font-medium">{{ __('Sign out') }}</button>
        </form>
    </div>
</div>