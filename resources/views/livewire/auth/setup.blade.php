        ...
        <form wire:submit="completeSetup" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative" x-data="{ open: false }" @keydown.escape.window="open = false">
                    <input
                        wire:model.live.debounce.250ms="school"
                        id="school"
                        type="text"
                        @focus="open = true"
                        @input="open = true"
                        ...
                        placeholder="{{ __('Type your university name') }}"
                        autocomplete="off"
                    >
                    ...
                    @if(count($this->schoolSuggestions) > 0)
                        <div x-show="open" x-cloak @click.outside="open = false"
                            ...
                            @foreach($this->schoolSuggestions as $suggestion)
                                <button type="button"
                                        wire:click="$set('school', @js($suggestion))"
                                        @click="open = false"
                ...
                <p class="mt-1 text-xs text-gray-500">{{ __('You can type to search or enter your own institution name.') }}</p>
                @error('school') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
            </div>

            <div>
                <label id="study-year-dropdown-label" class="block text-sm font-medium text-gray-700 mb-1">{{ __('Study Year') }}</label>
                <div class="relative dropdown-menu" id="study-year-dropdown" x-data="{ open: false }">
                    <button type="button"
                            @click="open = !open"
                            aria-haspopup="menu"
                            aria-controls="study-year-dropdown-menu-list"
                    ...
                    <div id="study-year-dropdown-menu-popover" data-popover x-show="open" :aria-hidden="open ? 'false' : 'true'" x-cloak @click.outside="open = false"
                         x-transition:enter="transition ease-out duration-100"
                         x-transition:enter-start="opacity-0 scale-95"
                         x-transition:enter-end="opacity-100 scale-100"
                         x-transition:leave="transition ease-in duration-75"
                         ...
                         x-transition:leave-end="opacity-0 scale-95"
                         class="absolute right-0 mt-2 w-full bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20 max-h-64 overflow-y-auto">
                        <div role="menu" id="study-year-dropdown-menu-list" aria-labelledby="study-year-dropdown-label" class="outline-none">
                ...
                @if($study_year === 'Other')
                    <input wire:model="study_year_other" type="text" class="mt-3 block w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="{{ __('Enter your study year') }}">
                    @error('study_year_other') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                @endif
            </div>

            <div>
                <label for="birth_date" class="block text-sm font-medium text-gray-700 mb-1">{{ __('Date of Birth') }}</label>
                <input wire:model="birth_date" type="date" id="birth_date" class="block w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                @error('birth_date') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
            </div>

            <label class="flex items-start">
                <input wire:model="accept_tos" type="checkbox" class="mt-1 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                <span class="ml-2 text-sm text-gray-600">
                    {{ __('I agree to the') }}
                    <a href="{{ route('tos') }}" class="text-indigo-600 hover:text-indigo-500 font-medium">{{ __('Terms of Service') }}</a>
                </span>
            </label>
            @error('accept_tos') <p class="-mt-3 text-sm text-red-500">{{ $message }}</p> @enderror

            <button type="submit" class="w-full flex justify-center items-center py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm transition-colors">
                <span wire:loading.remove wire:target="completeSetup">{{ __('Complete Setup') }}</span>
                <span wire:loading wire:target="completeSetup"><i class="fas fa-spinner fa-spin mr-2"></i>{{ __('Saving...') }}</span>
            </button>
        </form>
