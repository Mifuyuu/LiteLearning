...
<div class="max-w-4xl mx-auto space-y-6 ">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[#101114] flex items-center gap-2"> {{ 'ร้านค้าของตกแต่ง' }}</h1>
            <p class="text-sm text-[#9497a9] mt-1">{{ 'ใช้เหรียญแลกของตกแต่งสุดพิเศษ' }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl border border-[#dedee5] shadow-[rgba(0,0,0,0.03)_0px_4px_24px] p-6 mb-6">
            ...
            <div class="flex space-x-6 overflow-x-auto pb-4 mb-8 store-scrollbar" @wheel="if ($event.deltaY !== 0) { $event.preventDefault(); $el.scrollLeft += $event.deltaY; }">
                @foreach($storeItems->where('type', 'name_color') as $item)
                    ...
                    @endphp
                    <div class="w-[240px] shrink-0 border rounded-xl p-5 flex flex-col items-center text-center transition-all {{ $isOwned ? 'border-[rgba(37,99,235,0.4)] bg-[var(--ll-blue-hover)]' : 'border-[#dedee5] hover:border-[rgba(37,99,235,0.3)] hover:shadow-[rgba(0,0,0,0.06)_0px_4px_24px]' }}">
                        ...
                        <div class="mt-4 w-full">
                            @if($isOwned)
                                <a href="{{ route('inventory') }}" wire:navigate
                                    class="inline-flex items-center justify-center gap-1 w-full py-2.5 bg-white border border-[#d1d5db] text-[#686b82] hover:bg-gray-50 font-medium rounded-[8px] text-sm transition-colors">
